# -*- coding: utf-8 -*-
"""
Forest Classification Tool - Phase 1: Basic Toolbox Structure v0.1.5

Created: 2025-08-28 14:45
Version: 0.1.5
...
class ForestClassificationToolbox(object):
    ...
    def __init__(self):
        ...
        self.label = "Forest Classification Toolbox - Phase 1 v0.1.5"
        self.alias = "ForestClassificationPhase1v0_1_5"
...
class ForestClassificationTool(object):
    ...
    def __init__(self):
        ...
        self.label = "Forest Classification Tool - Phase 1 v0.1.5"
    ...
    def getParameterInfo(self):
        ...
        # Set up filter with better error handling
        try:
            if hasattr(thread_config, "filter") and thread_config.filter is not None:
                thread_config.filter.type = "ValueList"
                thread_config.filter.list = thread_options
            else:
                # Create a new filter if it doesn't exist
                thread_config.filter = arcpy.Parameter().filter
                thread_config.filter.type = "ValueList"
                thread_config.filter.list = thread_options
        except (AttributeError, Exception):
            # Fallback: just set the default value
            pass
        ...
        # Set up filter with better error handling
        try:
            if hasattr(memory_config, "filter") and memory_config.filter is not None:
                memory_config.filter.type = "ValueList"
                memory_config.filter.list = memory_options
            else:
                # Create a new filter if it doesn't exist
                memory_config.filter = arcpy.Parameter().filter
                memory_config.filter.type = "ValueList"
                memory_config.filter.list = memory_options
        except (AttributeError, Exception):
            # Fallback: just set the default value
    ...
    def execute(self, parameters, messages):
        ...
        # Basic logging to ArcGIS Pro messages
        arcpy.AddMessage("🚀 Starting Forest Classification Tool - Phase 1 v0.1.5")
        arcpy.AddMessage(
            "📋 Phase 1 v0.1.5: Fixed header format with full datetime and improved ToolValidator approach"
        )
